// Mocks generated by Mockito 5.3.2 from annotations
// in satagro/test/features/sat_agro/data/repositories/field_repository_impl_test.dart.
// Do not manually edit this file.

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'dart:async' as _i4;

import 'package:mockito/mockito.dart' as _i1;
import 'package:satagro/core/network/network_info.dart' as _i3;
import 'package:satagro/features/sat_agro/data/data_sources/field_local_data_source.dart'
    as _i6;
import 'package:satagro/features/sat_agro/data/data_sources/field_remote_data_source.dart'
    as _i5;
import 'package:satagro/features/sat_agro/data/data_transfer_objects/field_DTO.dart'
    as _i2;

// ignore_for_file: type=lint
// ignore_for_file: avoid_redundant_argument_values
// ignore_for_file: avoid_setters_without_getters
// ignore_for_file: comment_references
// ignore_for_file: implementation_imports
// ignore_for_file: invalid_use_of_visible_for_testing_member
// ignore_for_file: prefer_const_constructors
// ignore_for_file: unnecessary_parenthesis
// ignore_for_file: camel_case_types
// ignore_for_file: subtype_of_sealed_class

class _FakeFieldDTO_0 extends _i1.SmartFake implements _i2.FieldDTO {
  _FakeFieldDTO_0(
    Object parent,
    Invocation parentInvocation,
  ) : super(
          parent,
          parentInvocation,
        );
}

/// A class which mocks [NetworkInfo].
///
/// See the documentation for Mockito's code generation for more information.
class MockNetworkInfo extends _i1.Mock implements _i3.NetworkInfo {
  MockNetworkInfo() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i4.Future<bool> get isConnected => (super.noSuchMethod(
        Invocation.getter(#isConnected),
        returnValue: _i4.Future<bool>.value(false),
      ) as _i4.Future<bool>);
}

/// A class which mocks [FieldRemoteDataSource].
///
/// See the documentation for Mockito's code generation for more information.
class MockFieldRemoteDataSource extends _i1.Mock
    implements _i5.FieldRemoteDataSource {
  MockFieldRemoteDataSource() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i4.Future<_i2.FieldDTO>? getField(int? id) =>
      (super.noSuchMethod(Invocation.method(
        #getField,
        [id],
      )) as _i4.Future<_i2.FieldDTO>?);
}

/// A class which mocks [FieldLocalDataSource].
///
/// See the documentation for Mockito's code generation for more information.
class MockFieldLocalDataSource extends _i1.Mock
    implements _i6.FieldLocalDataSource {
  MockFieldLocalDataSource() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i4.Future<_i2.FieldDTO> getLastField() => (super.noSuchMethod(
        Invocation.method(
          #getLastField,
          [],
        ),
        returnValue: _i4.Future<_i2.FieldDTO>.value(_FakeFieldDTO_0(
          this,
          Invocation.method(
            #getLastField,
            [],
          ),
        )),
      ) as _i4.Future<_i2.FieldDTO>);
  @override
  _i4.Future<void>? cacheField(_i2.FieldDTO? fieldDTO) => (super.noSuchMethod(
        Invocation.method(
          #cacheField,
          [fieldDTO],
        ),
        returnValueForMissingStub: _i4.Future<void>.value(),
      ) as _i4.Future<void>?);
}

/// A class which mocks [FieldRemoteDataSource].
///
/// See the documentation for Mockito's code generation for more information.
class MockRemoteDataSource extends _i1.Mock
    implements _i5.FieldRemoteDataSource {
  @override
  _i4.Future<_i2.FieldDTO>? getField(int? id) => (super.noSuchMethod(
        Invocation.method(
          #getField,
          [id],
        ),
        returnValueForMissingStub: null,
      ) as _i4.Future<_i2.FieldDTO>?);
}

/// A class which mocks [FieldLocalDataSource].
///
/// See the documentation for Mockito's code generation for more information.
class MockLocalDataSource extends _i1.Mock implements _i6.FieldLocalDataSource {
  @override
  _i4.Future<_i2.FieldDTO> getLastField() => (super.noSuchMethod(
        Invocation.method(
          #getLastField,
          [],
        ),
        returnValue: _i4.Future<_i2.FieldDTO>.value(_FakeFieldDTO_0(
          this,
          Invocation.method(
            #getLastField,
            [],
          ),
        )),
        returnValueForMissingStub:
            _i4.Future<_i2.FieldDTO>.value(_FakeFieldDTO_0(
          this,
          Invocation.method(
            #getLastField,
            [],
          ),
        )),
      ) as _i4.Future<_i2.FieldDTO>);
  @override
  _i4.Future<void>? cacheField(_i2.FieldDTO? fieldDTO) => (super.noSuchMethod(
        Invocation.method(
          #cacheField,
          [fieldDTO],
        ),
        returnValueForMissingStub: _i4.Future<void>.value(),
      ) as _i4.Future<void>?);
}
